...
func (ep *EndPoint) sendFlitOut(now sim.VTimeInSec) bool {
	...
	for i := 0; i < ep.numOutputChannels; i++ {
		...
		if err == nil {
			...
			// 	if test.VAddr == uint64(4800512) {
			// 		fmt.Printf("%0.9f,%s,sendFlitOut,%s,%d\n", float64(now), ep.Name(), test.TaskID, test.VAddr)
			// 	}
			// }
...
func (ep *EndPoint) prepareFlits(_ sim.VTimeInSec) bool {
	...
	for {
		...
		// // case *vm.TranslationReq:
		// // 	if test.VAddr == uint64(4800512) {
		// // 		fmt.Printf("%0.9f,%s,prepareFlits,%s,%d\n", float64(ep.Engine.CurrentTime()), ep.Name(), test.TaskID, test.VAddr)
		// // 	}
		// }
		// // fmt.Printf("%.10f, %s, ep send, msg-%s, %d\n",
		// // 	ep.Engine.CurrentTime(), ep.Name(), msg.Meta().ID,
		// // 	len(ep.flitsToSend))
		// // if msg.Meta().ID == "5540302" {
		// // 	fmt.Println("msg-5540302")
		// // }
...
func (ep *EndPoint) tryDeliver(now sim.VTimeInSec) bool {
	...
	for len(ep.assembledMsgs) > 0 {
		...
		// switch test := msg.(type) {
		// case *vm.TranslationReq:
		// 	fmt.Printf("%0.9f,%s TranslationReq,%s,%d\n", float64(now), ep.Name(), test.TaskID, test.VAddr)
		// 	if test.VAddr == uint64(4800512) {
		// 		if err != nil {
		// 			fmt.Println("4800512, err")
		// 		} else {
		// 			fmt.Println("4800512, delivered")
		// 		}
		// 	}
		// }
