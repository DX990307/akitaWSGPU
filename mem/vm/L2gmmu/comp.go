...
type GMMU struct {
	...
	// remoteMemReqs       map[string]map[uint64]transaction
...
func (gmmu *GMMU) walkPageTable(now sim.VTimeInSec) bool {
	...
	for i := 0; i < len(gmmu.walkingTranslations); i++ {
		...
		if contains(gmmu.DeviceIDList, page.DeviceID) {
...
func (gmmu *GMMU) processRemoteMemReq(now sim.VTimeInSec, walkingIndex int) bool {
	...
	// gmmu.remoteMemReqs[walking.Src.Name()][uint64(walking.VAddr)] = gmmu.walkingTranslations[walkingIndex]
...
func (gmmu *GMMU) handleTranslationRsp(now sim.VTimeInSec, rsponse *vm.TranslationRsp) bool {
	...
	// fmt.Printf("GMMU: %d\n", reqTransaction.req.VAddr)
	// if uint64(reqTransaction.req.VAddr) == 1196032 {
	// 	fmt.Printf("GMMU: %d\n", reqTransaction.req.VAddr)
	// }
